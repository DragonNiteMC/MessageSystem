...
public class IgnorePMCommand extends MSGSystemCommandNode {
    ...
    public void executionPlayer(ProxiedPlayer player, List<String> list) {
        if (list.size() < 1) {
            ...
            MessageBuilder.sendMessage(player, configManager.getMessage(ignoredPlayers.size() > 0 ? "msg.ignore.list" : "msg.ignore.none").replace("<player>", player.getDisplayName()).replace("<list>", ignoredPlayers.toString()));
