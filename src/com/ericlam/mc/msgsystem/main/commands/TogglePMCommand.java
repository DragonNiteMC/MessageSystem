...
public class TogglePMCommand extends MSGSystemCommandNode {
    ...
    public void executionPlayer(ProxiedPlayer player, List<String> list) {
        if (list.size() < 1) {
            ...
            final String targetPath = "msg.toggle.be-".concat(pmManager.isDisabledPM(target) ? "enable" : "disable");
            MessageBuilder.sendMessage(player, configManager.getMessage(path).replace("<toggled>", target.getDisplayName()));
            MessageBuilder.sendMessage(target, configManager.getMessage(targetPath).replace("<mod>", player.getDisplayName()));
        }
    }
